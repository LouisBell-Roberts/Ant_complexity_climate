...
ant_data <- read.csv("/Users/louis.bell-roberts/Documents/Github/Ant_complexity_climate/Supplementary_data/Data/AntTrait_Global_241027.csv")

#Transform temperature and rainfall with Z transformation
ant_data <- ant_data %>% 
  mutate(across(c(TMP_avg, PRE_avg), .fns = ~scale(.x), .names = "Z{.col}"))

#Select only the variables that I want to be in the PCA analysis
ant_data_cs_pca<- dplyr::select(ant_data, ZTMP_avg, ZPRE_avg)

#Must be in matrix format
ant_focal<- as.matrix(ant_data_cs_pca)

#Requires species names as row names
rownames(ant_focal) <- ant_data$species

#Read in phylogenies, one for each MCC tree - required for phylogenetic PCA
NCuniform_stem_tree <- read.tree("/Users/louis.bell-roberts/Documents/Github/Ant_complexity_climate/Supplementary_data/Trees/MCC_trees/NCuniform_stem_Global_pruned_MCC.tre")
NCuniform_crown_tree <- read.tree("/Users/louis.bell-roberts/Documents/Github/Ant_complexity_climate/Supplementary_data/Trees/MCC_trees/NCuniform_crown_Global_pruned_MCC.tre")
FBD_stem_tree<- read.tree("/Users/louis.bell-roberts/Documents/Github/Ant_complexity_climate/Supplementary_data/Trees/MCC_trees/FBD_stem_Global_pruned_MCC.tre")
FBD_crown_tree<- read.tree("/Users/louis.bell-roberts/Documents/Github/Ant_complexity_climate/Supplementary_data/Trees/MCC_trees/FBD_crown_Global_pruned_MCC.tre")
...
NCuniform_stem_PCA_data <- data.frame(ant_data,
                                      PC1= pca_mean_NCuniform_stem$S[,1][match(ant_data$species, rownames(pca_mean_NCuniform_stem$S))],
                                      PC2= pca_mean_NCuniform_stem$S[,2][match(ant_data$species, rownames(pca_mean_NCuniform_stem$S))])
...
NCuniform_crown_PCA_data <- data.frame(ant_data,
                                       PC1= pca_mean_NCuniform_crown$S[,1][match(ant_data$species, rownames(pca_mean_NCuniform_crown$S))],
                                       PC2= pca_mean_NCuniform_crown$S[,2][match(ant_data$species, rownames(pca_mean_NCuniform_crown$S))])
...
FBD_stem_PCA_data <- data.frame(ant_data,
                                PC1= pca_mean_FBD_stem$S[,1][match(ant_data$species, rownames(pca_mean_FBD_stem$S))],
                                PC2= pca_mean_FBD_stem$S[,2][match(ant_data$species, rownames(pca_mean_FBD_stem$S))])
...
FBD_crown_PCA_data <- data.frame(ant_data,
                                 PC1= pca_mean_FBD_crown$S[,1][match(ant_data$species, rownames(pca_mean_FBD_crown$S))],
                                 PC2= pca_mean_FBD_crown$S[,2][match(ant_data$species, rownames(pca_mean_FBD_crown$S))])
...
###Tropical
#NC_uniform_stem
ant_data_tropical_PCA_NCuniform_stem <- NCuniform_stem_PCA_data %>% dplyr::filter(species %in% ant_data_tropical$species)
#NC_uniform_crown
ant_data_tropical_PCA_NCuniform_crown <- NCuniform_crown_PCA_data %>% dplyr::filter(species %in% ant_data_tropical$species)
#FBD_stem
ant_data_tropical_PCA_FBD_stem <- FBD_stem_PCA_data %>% dplyr::filter(species %in% ant_data_tropical$species)
#FBD_crown
ant_data_tropical_PCA_FBD_crown <- FBD_crown_PCA_data %>% dplyr::filter(species %in% ant_data_tropical$species)

###Temperate
#NC_uniform_stem
ant_data_temperate_PCA_NCuniform_stem <- NCuniform_stem_PCA_data %>% dplyr::filter(species %in% ant_data_temperate$species)
#NC_uniform_crown
ant_data_temperate_PCA_NCuniform_crown <- NCuniform_crown_PCA_data %>% dplyr::filter(species %in% ant_data_temperate$species)
#FBD_stem
ant_data_temperate_PCA_FBD_stem <- FBD_stem_PCA_data %>% dplyr::filter(species %in% ant_data_temperate$species)
#FBD_crown
ant_data_temperate_PCA_FBD_crown <- FBD_crown_PCA_data %>% dplyr::filter(species %in% ant_data_temperate$species)


###Both
#NC_uniform_stem
ant_data_both_PCA_NCuniform_stem <- NCuniform_stem_PCA_data %>% dplyr::filter(species %in% ant_data_both$species)
#NC_uniform_crown
ant_data_both_PCA_NCuniform_crown <- NCuniform_crown_PCA_data %>% dplyr::filter(species %in% ant_data_both$species)
#FBD_stem
ant_data_both_PCA_FBD_stem <- FBD_stem_PCA_data %>% dplyr::filter(species %in% ant_data_both$species)
#FBD_crown
ant_data_both_PCA_FBD_crown <- FBD_crown_PCA_data %>% dplyr::filter(species %in% ant_data_both$species)
